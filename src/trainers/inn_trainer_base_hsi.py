...
class DAInnBaseHSI(DomainAdaptationTrainerBaseHSI, ABC):
    ...
    def sample_inverted_spectrum(self, spectrum, mode="a", visualize=False):
        ...
        if visualize:
            plt.plot(orig_spectrum, label="input spectrum")
            plt.plot(z.cpu().detach().numpy(), label="latent variable")
            plt.plot(inverted_spectrum, label="Inverse Image")
            plt.show()
        else:
            plt.close()
    ...
    def gan_inn_training_step(self, batch, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            gen_a_loss = self.discriminator_a.calc_gen_loss(spectra_ba)
            gen_b_loss = self.discriminator_b.calc_gen_loss(spectra_ab)
            ...
            dis_a_loss = self.discriminator_a.calc_dis_loss(spectra_ba.detach(), spectra_a)
            dis_b_loss = self.discriminator_b.calc_dis_loss(spectra_ab.detach(), spectra_b)
