...
def load_data():
    ...
    file = settings.results_dir / 'inn' / 'inn_output.npz'
    inn_data = np.load(file, allow_pickle=True)
    data_c = torch.tensor(inn_data['spectra_ab'])
    # spectra adapted from synthetic to real should be normalized with the statistics of the real data set
    data_c = ((data_c / torch.linalg.norm(data_c, ord=2, dim=1).unsqueeze(dim=1)) - tdl.dataset.exp_config.data["mean_b"]) / tdl.dataset.exp_config.data["std_b"]
    seg_c = torch.tensor(inn_data['seg_a'])
